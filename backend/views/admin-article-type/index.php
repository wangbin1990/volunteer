...
use yii\helpers\Url;
$modelLabel = new \backend\models\AdminArticleType();
?>
        ...
        <div class="box-body">
          <div id="example1_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
            <!-- row start search-->
          	<div class="row">
          	<div class="col-sm-12">
                <?php ActiveForm::begin(['id' => 'admin-article-type-search-form', 'method'=>'get', 'options' => ['class' => 'form-inline'], 'action'=>Url::toRoute('admin-article-type/index')]); ?>     
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
                <?php $form = ActiveForm::begin(["id" => "admin-article-type-form", "class"=>"form-horizontal", "action"=>"index.php?r=admin-article-type/save"]); ?>                      
                 
          <input type="hidden" class="form-control" id="id" name="AdminArticleType[id]" />
          ...
          <div id="controller_id_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="name" name="AdminArticleType[name]" placeholder="必填" />
              </div>
          ...
          <div id="action_id_div" class="form-group">
              ...
              <div class="col-sm-10">
<!--                   <input type="radio" class="form-control"  name="AdminArticleType[status]" value="1" checked="checked" />是
                  <input type="radio" class="form-control"  name="AdminArticleType[status]" value="0" />否 -->
                  <select id="status" name="AdminArticleType[status]">
                    <option value="" <?php if ("AdminArticleType[status]" == '') { ?>selected="selected" <?php } ?>>请选择</option>
                    <option value="1" <?php if ("AdminArticleType[status]" == 1) { ?>selected="selected" <?php } ?>>是</option>
                    <option value="2" <?php if ("AdminArticleType[status]" == 2) { ?>selected="selected" <?php } ?>>否</option>
                  </select>
              </div>
              <div class="clearfix"></div>
          </div>
          ...
          <div id="url_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="sort" name="AdminArticleType[sort]" placeholder="" />
              </div>
              <div class="clearfix"></div>
          </div>
          ...
          <div id="create_user_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="create_user" name="AdminArticleType[create_user]" placeholder="" />
              </div>
              <div class="clearfix"></div>
          </div>
          ...
          <div id="create_date_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="create_date" name="AdminArticleType[create_date]" placeholder="" />
              </div>
              <div class="clearfix"></div>
          </div>
          ...
          <div id="update_user_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="update_user" name="AdminArticleType[update_user]" placeholder="" />
              </div>
              <div class="clearfix"></div>
          </div>
          ...
          <div id="update_date_div" class="form-group">
              ...
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="update_date" name="AdminArticleType[update_date]" placeholder="" />
              </div>
              <div class="clearfix"></div>
          </div>
...
function orderby(field, op){
 ...
 function searchAction(){
		$('#admin-article-type-search-form').submit();
	}
...
function initModel(id, type, fun){
	...
	$.ajax({
		   type: "GET",
		   url: "<?=Url::toRoute('admin-article-type/view')?>",
		   data: {"id":id},
		   cache: false,
		   dataType:"json",
...
function deleteAction(id){
	...
	if(ids.length > 0){
		admin_tool.confirm('请确认是否删除', function(){
		    $.ajax({
				   type: "GET",
				   url: "<?=Url::toRoute('admin-article-type/delete')?>",
				   data: {"ids":ids},
				   cache: false,
				   dataType:"json",
				   error: function (xmlHttpRequest, textStatus, errorThrown) {
...
$('#edit_dialog_ok').click(function (e) {
    e.preventDefault();
	$('#admin-article-type-form').submit();
});
...
$('#admin-article-type-form').bind('submit', function(e) {
	e.preventDefault();
	var id = $("#id").val();
	var action = id == "" ? "<?=Url::toRoute('admin-article-type/create')?>" : "<?=Url::toRoute('admin-article-type/update')?>";
    $(this).ajaxSubmit({
    	type: "post",
    	dataType:"json",
    	url: action,
