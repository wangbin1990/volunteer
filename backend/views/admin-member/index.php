...
//               echo '<th class="sorting" tabindex="0" aria-controls="data_table" rowspan="1" colspan="1" aria-sort="ascending" >'.$modelLabel->getAttributeLabel('auth_key').'</th>';
              ...
              echo '<th onclick="orderby(\'id\', \'desc\')" '.CommonFun::sortClass($orderby, 'num').' tabindex="0" aria-controls="data_table" rowspan="1" colspan="1" aria-sort="ascending" >'.$modelLabel->getAttributeLabel('num').'</th>';
              echo '<th onclick="orderby(\'id\', \'desc\')" '.CommonFun::sortClass($orderby, 'status').' tabindex="0" aria-controls="data_table" rowspan="1" colspan="1" aria-sort="ascending" >'.$modelLabel->getAttributeLabel('status').'</th>';
...
//               echo '<th class="sorting" tabindex="0" aria-controls="data_table" rowspan="1" colspan="1" aria-sort="ascending" >'.$modelLabel->getAttributeLabel('create_date').'</th>';
              ...
              echo '<th onclick="orderby(\'id\', \'desc\')" '.CommonFun::sortClass($orderby, 'update_date').' tabindex="0" aria-controls="data_table" rowspan="1" colspan="1" aria-sort="ascending" >'.$modelLabel->getAttributeLabel('update_date').'</th>';
...
<div class="modal fade" id="adds" tabindex="-1" role="dialog"
  aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
...
<!--           <div id="password_div" class="form-group">
              <label for="password" class="col-sm-2 control-label">通用密码</label>
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="addpassword" name="AdminMember[password]" placeholder="必填" />
              </div>
              <div class="clearfix"></div>
          </div> -->
          ...
          <div id="status_div" class="form-group">
              <label for="status" class="col-sm-2 control-label">生成个数</label>
              <div class="col-sm-10">
                  <input type="text" class="form-control" id="addstatus" name="AdminMember[status]" placeholder="必填" />
              </div>
              <div class="clearfix"></div>
          </div>
      <?php ActiveForm::end(); ?>          
      ...
      <div class="modal-footer">
        <a href="#" class="btn btn-default" data-dismiss="modal">关闭</a> <a
          id="edit_dialog_oks" href="#" class="btn btn-primary">确定</a>
      </div>
    </div>
  </div>
</div>
...
function initModel(id, type, fun){
	...
	$.ajax({
		   type: "GET",
		   url: "<?=Url::toRoute('admin-member/view')?>",
		   data: {"id":id},
		   cache: false,
		   dataType:"json",
...
function deleteAction(id){
	...
	if(ids.length > 0){
		admin_tool.confirm('请确认是否删除', function(){
		    $.ajax({
				   type: "GET",
				   url: "<?=Url::toRoute('admin-member/delete')?>",
				   data: {"ids":ids},
				   cache: false,
				   dataType:"json",
				   error: function (xmlHttpRequest, textStatus, errorThrown) {
...
$('#edit_dialog_oks').click(function (e) {
    var aname = $('#addname').val();
    var alname = $('#addlname').val();
    var apassword = $('#addpassword').val();
    var astatus = $('#addstatus').val();
    if(aname == ""){
      alert("用户账号前缀不能为空！");
      return false;
    }
...
$('#admin-member-form').bind('submit', function(e) {
	e.preventDefault();
	var id = $("#id").val();
	var action = id == "" ? "<?=Url::toRoute('admin-member/create')?>" : "<?=Url::toRoute('admin-member/update')?>";
    $(this).ajaxSubmit({
    	type: "post",
    	dataType:"json",
    	url: action,
...
$('#admin-member-forms').bind('submit', function(e) {
  e.preventDefault();
  var id = $("#id").val();
  var action = id == "" ? "<?=Url::toRoute('admin-member/adds')?>" : "<?=Url::toRoute('admin-member/update')?>"; 
    $(this).ajaxSubmit({
      type: "post",
      dataType:"json",
      url: action,
