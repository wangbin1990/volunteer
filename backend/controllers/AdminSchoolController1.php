...
class AdminSchoolController extends BaseController
{
    ...
    public function actionAddScore()
    {
        ...
        // if (!empty(array_filter($data['year'])) && !empty(array_filter($data['high_score'])) && !empty(array_filter($data['agv_score']))) {
        if (!empty(array_filter($data['year']))) {
            ...
            //foreach($data['year'] as $key => $item) {
                ...
                //$score[$key ]['high_score'] = $data['high_score'][$key];
                //$score[$key ]['high_score'] = $data['high_score'][$key];
               //$score[$key ]['agv_score'] = $data['agv_score'][$key];
...
//$score[$key ]['batch_id'] = $data['batch_id'][$key];
          ...
          //  }
            ...
            $num = Yii::$app->db->createCommand()->batchInsert(
                'admin_school_score',
                ['year','school_id', 'low_score', 'high_score', 'agv_score', 'diff_score', 'plan_count', 'rank', 'batch_id', 'mold_id'],
                $score
            )->execute();
            return ['errno' => 0, 'message' => $num . '数据插入成功！'];
        }
        // return ['errno' => -1, 'message' => '年份，最高分，省控线必填'];
	}
